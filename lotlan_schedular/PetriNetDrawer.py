...
class PetriNetDrawer:
    ...
    def draw_arcs(self, arc, attr):
        if type(arc) == snakes.nets.Inhibitor:
            attr["arrowhead"] = DrawConstants.INHIBITOR_ARC_ARROW_HEAD
        attr["label"] = ""

    def draw_image(self, net, filename):
        net.draw(filename, DrawConstants.LAYOUT_METHOD, graph_attr=self.draw_graph, arc_attr=self.draw_arcs,
                 place_attr=self.draw_place, trans_attr=self.draw_transition)
